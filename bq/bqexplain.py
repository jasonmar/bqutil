...
def main():
    parser = argparse.ArgumentParser(description='Export BigQuery Query Plans.')
    parser.add_argument('project', type=str, default='', nargs='?', help='project id')
    parser.add_argument('start_days_ago', type=int, default=3, nargs='?', help='max days since query')
    parser.add_argument('end_days_ago', type=int, default=0, nargs='?', help='min days since query')
    args = parser.parse_args()
    if args.project == '':
        parser.print_help()
        print("Prints job statistics between start_days_ago and end_days_ago")
        sys.exit(1)
    bq = BQQueryPlanExporter(project=args.project)
